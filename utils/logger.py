...
class Logger():
    def __init__(self, opts):
        ...
        self.log_dir = os.path.join(opts.log_dir, opts.name)
    ...
    # def record_images(self, visuals, nrow=6, tag=['low-sr-gt','high_frequency-h-v','mask_ref-sr-gt']):
    #     imgs1 = []
    #     imgs2 = []
    #     imgs3 = []
    #     max_len = visuals[0].shape[0]
    ...
    #             continue
    #
    #         tmp1_imgs = [x[i] for x in visuals[0:2]]
    #         imgs1.append(np.hstack(tmp1_imgs))
    #
    #         tmp2_imgs = [y[i] for y in visuals[3:7]]
    #         imgs2.append(np.hstack(tmp2_imgs))
    #
    #         tmp3_imgs = [z[i] for z in visuals[8:-1]]
    #         imgs3.append(np.hstack(tmp3_imgs))
    #
    #     imgs1 = np.vstack(imgs1).astype(np.uint8)
    #     imgs2 = np.vstack(imgs2).astype(np.uint8)
    #     imgs3 = np.vstack(imgs3).astype(np.uint8)
    #
    #     self.writer.add_image(tag[0], imgs1, self.cur_iter, dataformats='HWC')
    #     self.writer.add_image(tag[1], imgs2, self.cur_iter, dataformats='HWC')
    #     self.writer.add_image(tag[2], imgs3, self.cur_iter, dataformats='HWC')
    ...
    def net_tec(self, net_input):
        self.writer.add_graph(net_input[0], net_input[1])

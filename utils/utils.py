...
def get_class_pix_percent(pic_rgb):
    ...
    for i in range(len(label_list)):
        ...
        percent.append(round(clas.shape[0]/(w*h), 4))
    return pixs, percent
...
def pic_to_tensor(img, size=None):
    """
...
# def HF_HV(img):
#     img = Image.fromarray(np.uint8(tensor_to_img(img))).convert('RGB')
#     srcA, (srcH, srcV, srcD) = pywt.dwt2(img, "haar")
#     srcH = Image.fromarray(np.uint8(srcH+srcV+srcD)).convert('RGB')
#     srcV = Image.fromarray(np.uint8(srcV)).convert('RGB')
#     srcH = pic_to_tensor(srcH)
#     srcV = pic_to_tensor(srcV)
#     return srcH,srcV

def HF_HV(PIC, vh=False):
    H = []
    L = []
    for i in range(PIC.shape[0]):
        img = Image.fromarray(np.uint8(tensor_to_img(PIC[i]))).convert('RGB')
        (b, g, r) = img.split()
        ...
        if vh:
            ...
            # L.append(pic_to_tensor(Image.merge('RGB', (Image.fromarray(bcA).convert('L'),
            #                                            Image.fromarray(gcA).convert('L'),
            #                                            Image.fromarray(rcA).convert('L')))))
            L.append(torch.unsqueeze(pic_to_tensor(Image.merge('RGB', (Image.fromarray(bcA).convert('L'),
                                                                       Image.fromarray(gcA).convert('L'),
                                                                       Image.fromarray(rcA).convert('L')))), dim=0))
            H.append(pic_to_tensor(Image.fromarray(bcV+gcV+rcV+bcH+gcH+rcH+bcD+gcD+rcD).convert('RGB')))
    return H, L

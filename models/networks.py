    ...
    for m in net.modules():
        if isinstance(m, nn.Conv2d):
            if weight_norm_type.lower() == 'spectral_norm':
                tutils.spectral_norm(m)  # 频谱归一化
            elif weight_norm_type.lower() == 'weight_norm':
                tutils.weight_norm(m)  # 权重归一化
            else:
                pass
...
def define_G(opt, isTrain=True, use_norm='none', relu_type='LeakyReLU',weight_path=None):
    net = jsfmnet.Generator_jsfmgan(3, 3, in_size=opt.Gin_size, out_size=opt.Gout_size, relu_type=relu_type, parse_ch=19, norm_type=opt.Gnorm)
    # net = jsfmnet.Generator_jsfmgan_simple(3, 3, in_size=opt.Gin_size, out_size=opt.Gout_size, relu_type=relu_type,
    #                                 parse_ch=19, norm_type=opt.Gnorm)
    #频谱归一化
...
class ParseNet(nn.Module):
    def __init__(self,
        ...
        down_steps = int(np.log2(in_size//min_feat_size))  # default=2
        up_steps = int(np.log2(out_size//min_feat_size))

        # =============== define encoder-body-decoder ==================== 
        self.encoder = []
        self.encoder.append(ConvLayer(3, base_ch, 3, 1)) # 3-64:pad+conv+pool+rule+normal
        head_ch = base_ch  # default=64
        for i in range(down_steps):
            cin, cout = ch_clip(head_ch), ch_clip(head_ch * 2)  # default=64:128->128:256

            self.encoder.append(ResidualBlock(cin, cout, scale='down', **act_args))# ConvLayer + 2ConvLayer
            head_ch = head_ch * 2

        self.body = []
        for i in range(res_depth):
            self.body.append(ResidualBlock(ch_clip(head_ch), ch_clip(head_ch), **act_args)) # 256,通道不变，ConvLayer + 2ConvLayer

        self.decoder = []
        for i in range(up_steps):
            cin, cout = ch_clip(head_ch), ch_clip(head_ch // 2) #256:128->128:64
            self.decoder.append(ResidualBlock(cin, cout, scale='up', **act_args)) # ConvLayer + 2ConvLayer
            head_ch = head_ch // 2
        ...
        self.out_img_conv = ConvLayer(ch_clip(head_ch), 3)  # 64 pad+conv+pool+rule+normal
        self.out_mask_conv = ConvLayer(ch_clip(head_ch), parsing_ch) # 64->19 pad+conv+pool+rule+normal

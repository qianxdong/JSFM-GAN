...
class ParseModel(BaseModel):
    ...
    def load_pretrain_models(self,):
        ...
        print('Loaded pretrained LQ face parsing network from', self.opt.parse_net_weight)
    ...
    def backward(self):
        # self.pred_Parse =torch.squeeze(self.pred_Parse, 0)
        # self.gt_Parse = torch.squeeze(self.gt_Parse, 0)

        # self.pred_Parse = self.pred_Parse.view(1, 19, 512, 512)
        # self.gt_Parse = self.gt_Parse.view(3, 1, 512, 512)
        #
        # self.pred_Parse = self.pred_Parse.expand(3, 19, 512, 512)
        # self.gt_Parse = self.gt_Parse.expand(3, 19, 512, 512)
        #
        # self.pred_Parse = self.pred_Parse.reshape(57, 512, 512)
        # self.gt_Parse = self.gt_Parse.reshape(57, 512, 512)

        self.gt_Parse = self.gt_Parse.argmax(dim=-1)

        # print('gt_Parse', self.gt_Parse.shape)

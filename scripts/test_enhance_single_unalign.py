...
def detect_and_align_faces(img, face_detector, lmk_predictor, template_path, template_scale=2, size_threshold=999):
    ...
    print(f'Detect landmark points')
...
def adjust_map(maps):
        ...
        if area / (maps.shape[2] * maps.shape[3]) < 0.04 or area1 == 0 or area2 == 0:
            skin = skin + eye_g
            eye_g[eye_g > 0] = 0
            maps[:, 1:2, :, :] = skin
            maps[:, 3:4, :, :] = eye_g
...
def enhance_faces(LQ_faces, model):
    ...
    for lq_face in tqdm(LQ_faces):
        with torch.no_grad():
            ...
            parse_map_onehot = adjust_map(parse_map_onehot)
...
if __name__ == '__main__':
    ...
    print('======> dlib.load_rgb_image over.')
